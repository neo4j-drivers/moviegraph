...
def match_similar_movies(tx, title):
    return tx.run("MATCH (movie:Movie)<-[]-(person:Person)-[]->(similar:Movie) "
                  "WHERE movie.title = $title "
                  "WITH similar, count(person) AS similarity "
                  "RETURN similar ORDER BY similarity DESCENDING", title=title).value()
...
def get_movie(title):
    ...
    with driver.session() as session:
        ...
        similar = session.read_transaction(match_similar_movies, title)
    if record is None:
        return abort(404, "Movie not found")
    return render_template("movie.html", movie=record["movie"], actors=record["actors"], similar=similar)
